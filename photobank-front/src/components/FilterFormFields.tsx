...
export const FilterFormFields = ({ control }: FilterFormFieldsProps) => {
  ...
  const paths = useSelector((state: RootState) => state.metadata.paths).map(
    (s) => {
      return { label: s.path, value: s.path };
    }
  );
  const persons = useSelector((state: RootState) => state.metadata.persons).map(
    (s) => {
      return { label: s.name, value: s.id.toString() };
    }
  );
  const tags = useSelector((state: RootState) => state.metadata.tags).map(
    (s) => {
      return { label: s.name, value: s.id.toString() };
    }
  );
