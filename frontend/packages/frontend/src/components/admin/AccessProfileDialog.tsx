...
import { Plus, Save, Trash2, X, ChevronDownIcon } from 'lucide-react';
...
const DEFAULT_FORM_VALUES: AccessProfileFormValues = {
  name: '',
  description: '',
...
interface AccessProfileDialogProps {
  ...
  initialProfile?: AccessProfileDto | null;
  title: string;
  submitLabel: string;
  ...
  missingProfileToast?: ToastConfig;
}
...
export function AccessProfileDialog({
  ...
  const lastSubmittedValuesRef = useRef<AccessProfileFormValues | null>(null);
  ...
  useEffect(() => {
    if (mode === 'edit' && open) {
      ...
      form.reset(mapProfileToFormValues(initialProfile));
    }
  }, [mode, open, initialProfile, form]);

  const createMutation = useAdminAccessProfilesCreate(
    mode === 'create'
      ? {
          mutation: {
            onSuccess: async () => {
              await queryClient.invalidateQueries({
                queryKey: accessProfilesQueryKey,
              });

              if (lastSubmittedValuesRef.current) {
                const toastConfig = successToast({
                  values: lastSubmittedValuesRef.current,
                  profile: undefined,
                });
                toast(toastConfig);
              }
            ...
            onError: () => {
              toast({ ...errorToast, variant: errorToast.variant ?? 'destructive' });
            },
          },
        }
      : undefined
  ...
  const updateMutation = useAdminAccessProfilesUpdate(
    mode === 'edit'
      ? {
          mutation: {
            onSuccess: async () => {
            ...
            onSettled: () => {
              form.reset(
                initialProfile
                  ? mapProfileToFormValues(initialProfile)
                  : DEFAULT_FORM_VALUES
              );
              onOpenChange(false);
            },
  ...
  const addDateRange = () => {
    ...
    form.setValue('dateRanges', updatedRanges, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
  ...
  const updateDateRange = (index: number, updatedRange: DateRangeFormValue) => {
    ...
    form.setValue('dateRanges', nextRanges, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
    ...
    if (mode === 'create') {
      ...
      try {
        await createMutation.mutateAsync({
          data: payload as AccessProfileDto,
        });
      } catch {
        // Error handling is managed within mutation callbacks.
      }
      return;
    }

    if (!initialProfile?.id) {
      const fallbackToast: ToastConfig =
        missingProfileToast ?? {
      ...
      toast({ ...fallbackToast, variant: fallbackToast.variant ?? 'destructive' });
      return;
    }

    const payload = buildUpdatePayload(values, initialProfile);
                          ...
                          storages.map((storage) => (
                            <div key={storage.id} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                disabled={storagesQuery.isLoading || isSubmitting}
                                onCheckedChange={(checked) => {
                                  ...
                                  if (checked === true) {
                                    ...
                                    field.onChange(
                                      currentValue.filter((value) => value !== storage.id)
                                    );
                                  }
                                }}
                              />
                          ...
                          personGroups.map((group) => (
                            <div key={group.id} className="flex items-center space-x-2">
                              <Checkbox
                                ...
                                disabled={personGroupsQuery.isLoading || isSubmitting}
                                onCheckedChange={(checked) => {
                                  const currentValue = field.value ?? [];
                                  if (checked === true) {
                                    ...
                                    field.onChange(
                                      currentValue.filter((value) => value !== group.id)
                                    );
                                  }
                                }}
                              />
...
function DateRangeRow({ index, range, onChange, onRemove, disabled }: DateRangeRowProps) {
  ...
  const formattedFrom = range.fromDate
    ? format(range.fromDate, 'yyyy-MM-dd')
    : 'Select date';
  const formattedTo = range.toDate
...
function mapProfileToFormValues(profile: AccessProfileDto): AccessProfileFormValues {
  const storages = profile.storages?.flatMap((storage) =>
    typeof storage.storageId === 'number' ? [storage.storageId] : []
  );

  const personGroups = profile.personGroups?.flatMap((group) =>
    typeof group.personGroupId === 'number' ? [group.personGroupId] : []
  );

  const dateRanges = profile.dateRanges?.flatMap((range) => {
  ...
  } satisfies AccessProfileFormValues;
}

function buildCreatePayload(values: AccessProfileFormValues): AccessProfileDraft {
  const normalizedDateRanges =
    values.dateRanges?.flatMap((range) => {
      if (
  ...
  return {
    ...
    personGroups:
      values.personGroups?.map((personGroupId) => ({ personGroupId })) ?? [],
    dateRanges: normalizedDateRanges,
  } satisfies AccessProfileDraft;
}
...
function buildUpdatePayload(
  ...
  return {
    ...
    storages:
      values.storages?.map((storageId) => ({
        profileId: profile.id,
        storageId,
      })) ?? [],
    personGroups:
      values.personGroups?.map((personGroupId) => ({
        profileId: profile.id,
        personGroupId,
      })) ?? [],
    dateRanges: normalizedDateRanges,
